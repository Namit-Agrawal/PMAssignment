...
The excel guru is right about removing buildings with less than 10%  occupancy
as based on the summary below, it looks like there is only 1 green building to consider
, very few class a buildings and half the building have only 3 stories hence
...
Use median for stories since there are some outliers as suggested by the plot below.
It looks there is not much evidence for confounding going on with stories even though there is slight increase in rent as the number of stories goes up. The median for stories
for green building is only 1 higher so stories may not directly be affecting the rent.
```{r echo=FALSE}
...
It looks like there is no confounding for age as there is no correlation between
...
with rent from plot below and the median size is double for green buildings versus non green. Thus, there is a premium for size when it comes to rent as expected.
```{r echo=FALSE}
...
rent is the same regardless if its green or not, however cluster rent is highly
...
It looks like class a buildings are a confounding variable as class a buildings
correlate with green_rating and class a buildings correlate with overall rent as
well.
```{r echo=FALSE}
...
**Thoughts**\
From the investigation above it seems like size and class_a buildings are the only
confounding variables as the green buildings tend to have larger spaces
and larger spaces have higher rent. The excel guru here is only taking into account
the median rent of all the building with more than 10% occupancy, however if we apply
another filter to include only 15 story buildings, we see that the rent goes up 
drastically for green buildings all the way to 37 dollars. However, it may not
be wise to use this as there is only 10 green buildings that have 15 stories.
After thorough analysis it seems like the excel guru is correct about
recuperating the costs in a little over 8 years. 
```{r echo=FALSE}
