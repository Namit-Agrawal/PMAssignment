...
ggplot(sorted, aes(x=reorder(Category,PC4), y=PC4)) +
  ...
  ylab("Category")
```
It is important to note also that both PCs have pushed the category spam and uncategorized generally towards 0.  \
...
From this graph, we can conclude that the groups that follow NutrientH20 are generally people who either love fitness and health or are college students, with possible combinations of both.  The points that lie in the middle are typically going to be uncategorized or spam, as both PCs push those towards 0.  Another interesting group is the group of points that lie in the lower left corner.  Since travel and politics are negative in both PCs, we suspect that they may just be news outlets or journalists.
...
We then fit a k-Nearest Neighbors model on our training dataset.  Based on the plot below, we would choose to use $k = 1$ to maximize accuracy - however, due to the bias-variance tradeoff, this would not be an optimal choice.  A $k$ of 6 would help balance the tradeoff with a loss of accuracy.
...
```{r warning = FALSE,message = FALSE,include = FALSE,fig.align = "center",fig.width = 6,fig.height = 4}
basket.rules <- apriori(baskets.trans, parameter=list(support=.001, confidence=.1, maxlen=3))
subs <- subset(basket.rules, subset=lift > 1)
```

```{r echo=FALSE, fig.align = "center",fig.width = 6,fig.height = 4}
arules::inspect(subs[1:5])
plot(head(subs, 20, by='lift'), method='graph')
```
...
subs <- subset(basket.rules2, subset=lift > 1)
```
...
```{r message = FALSE, warning= FALSE,echo = FALSE,fig.align = "center",fig.width = 6,fig.height = 4,warning = FALSE}
basket.rules3 <- apriori(baskets.trans, parameter=list(support=.02, confidence=.3, maxlen=3))
subs <- subset(basket.rules3, subset=lift > 2)
```
...
```{r echo = FALSE,message = FALSE, warning = FALSE,fig.align = "center",fig.width = 6,fig.height = 4,warning = FALSE}
basket.rules4 <- apriori(baskets.trans, parameter=list(support=.01, confidence=.3, maxlen=4))
subs <- subset(basket.rules4, subset=lift > 2)
arules::inspect(subs)
plot(head(subs, 20, by='lift'), method='graph')
```
