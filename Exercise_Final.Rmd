...
We first perform dimension reduction on the dataset to improve computational ability.  In addition, we can see which 
```{r}
pca.social <- prcomp(scaled)
points <- as.data.frame(c(summary(pca.social)$importance[3,]))
points$PC <- factor(rownames(points))
colnames(points)[1] <- "CumulativeVarianceExplained"
points <- data.frame(points[1:10,])
ggplot(data = points,aes(x = PC,y = CumulativeVarianceExplained)) + 
  geom_point()
```
